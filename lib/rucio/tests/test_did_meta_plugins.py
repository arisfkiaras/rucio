...
class TestDidMetaDidColumn():
    ...
    def test_add_did_meta(self):
        ...
        set_metadata(scope=tmp_scope, name=did_name, key='project', value='data12_8TeV')
        assert_equal(get_metadata(scope=tmp_scope, name=did_name)['project'], 'data12_8TeV')
...
class TestDidMetaJSON():
    ...
    def setup(self):
        self.implemented = JSONDidMeta().json_implemented(session)
    ...
    def test_add_did_meta(self):
        ...
        set_metadata(scope=tmp_scope, name=did_name, key=meta_key, value=meta_value, session=session)
        assert_equal(get_metadata(scope=tmp_scope, name=did_name, plugin='JSON', session=session)[meta_key], meta_value)
    ...
    def test_list_did_meta(self):
        ...
        if not self.implemented or session.bind.dialect.name == 'sqlite':
            # For some oracle and sqlite version json support is not implemented
            return
        tmp_scope = InternalScope('mock')
        root = InternalAccount('root')
        ...
        set_metadata(scope=tmp_scope, name=tmp_dsn1, key=meta_key1, value=meta_value1, session=session)

        tmp_dsn2 = 'dsn_%s' % generate_uuid()
        add_did(scope=tmp_scope, name=tmp_dsn2, type="DATASET", account=root, session=session)
        set_metadata(scope=tmp_scope, name=tmp_dsn2, key=meta_key1, value=meta_value2, session=session)

        tmp_dsn3 = 'dsn_%s' % generate_uuid()
        add_did(scope=tmp_scope, name=tmp_dsn3, type="DATASET", account=root, session=session)
        set_metadata(scope=tmp_scope, name=tmp_dsn3, key=meta_key2, value=meta_value1, session=session)

        tmp_dsn4 = 'dsn_%s' % generate_uuid()
        add_did(scope=tmp_scope, name=tmp_dsn4, type="DATASET", account=root, session=session)
        set_metadata(scope=tmp_scope, name=tmp_dsn4, key=meta_key1, value=meta_value1, session=session)
        set_metadata(scope=tmp_scope, name=tmp_dsn4, key=meta_key2, value=meta_value2, session=session)

        dids = list_dids(tmp_scope, {meta_key1: meta_value1}, session=session)
        results = []
        for d in dids:
            results.append(d)
        ...
        dids = list_dids(tmp_scope, {meta_key1: meta_value2}, session=session)
        results = []
        for d in dids:
            results.append(d)
        ...
        dids = list_dids(tmp_scope, {meta_key2: meta_value1}, session=session)
        results = []
        for d in dids:
            results.append(d)
        assert_equal(len(results), 1)
        ...
        dids = list_dids(tmp_scope, {meta_key1: meta_value1, meta_key2: meta_value2}, session=session)
        results = []
        for d in dids:
            results.append(d)
        assert_equal(len(results), 1)
...
class TestDidMetaClient():
    ...
    def setup(self):
        ...
        self.tmp_scope = 'mock'
        self.json_implemented = JSONDidMeta().json_implemented(session)
