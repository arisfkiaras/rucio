...
class SearchExtended(RucioController):
    ...
    def GET(self, scope):
        ...
        try:
            for did in list_dids_extended(scope=scope, filters=filters, type=type, long=long, recursive=recursive):
                yield dumps(did) + '\n'
        except UnsupportedOperation as error:
            raise generate_http_error(409, 'UnsupportedOperation', error.args[0])
        except KeyNotFound as error:
...
class Meta(RucioController):
    ...
    def GET(self, scope, name):
        ...
        try:
            ...
            meta = get_metadata(scope=scope, name=name, plugin=plugin)
