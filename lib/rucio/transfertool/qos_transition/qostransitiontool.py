...
class QoSTransitionTool(object):
    """
    Interface definition of the Rucio QoS transition tool
    """

    def __init__(self, external_host):
        """
        Initializes the QoS transition tool

        :param external_host:   The external host where the QoS transition tool API is running
        """
    ...
    def submit(self, files, job_params, timeout=None):
        ...
        :returns:            QoS transition tool internal identifiers.
        """
        pass
    ...
    def query(self, transfer_ids, details=False, timeout=None):
        """
        Query the status of transitions in the QoS transition tool.
